// The email arm's WIRING: the part that actually reaches Resend and the
// database.
//
// Split out of index.ts for the reason `push_deps.ts` was (Codex review on PR
// #85): importing index.ts executes `serveFunction` and binds a port, so
// nothing in it could ever be driven by a test. That blind spot has now hidden
// two defects in this exact file — the `claimSend` rule its own comment called
// load-bearing, and the missing request deadline (Codex, PR #86) — the
...
/**
 * A bound on one Resend request. Chosen the way `PUSH_TIMEOUT_MS` was, and it
 * must stay far below `fn_notification_claim_lease()`: a send that can outlive
 * its claim lets a second sender take the channel while the first is still
 * running, which is the duplicate this whole change exists to prevent.
 */
 ...
 *
 * index.ts does the `Deno.env` reads; this module takes values. That is what
 * makes it drivable at all — CI runs `deno test` with no permissions, so a
 * module that reads env at construction cannot be constructed in a test, and
 * the request options would go back to being untested (which is how the
 * missing deadline survived).
 */
...
export function makeSendDeps(cfg: SendConfig, fetchImpl: typeof fetch = fetch): SendDeps {
  const { db, apiKey, operatorId } = cfg;
  return {
    ...
    async sendEmail({ to, subject, html, headers }) {
      const res = await fetchImpl("https://api.resend.com/emails", {
        ...
        // did not — the sibling asymmetry this repository keeps recording.
        // It is load-bearing beyond tidiness: a request with no bound can
        // outlive its 5-minute claim lease, at which point a second sender
        // takes the channel and both are live, which is the duplicate email
        // send-once exists to prevent. Comfortably below the lease.
    ...
    async record(id, outcome, previousAttempts, stamp) {
      ...
      // FENCED on the claim stamp (0051). A sender whose lease lapsed while it
      // was still running has been replaced, and an unconditional write here
      // would clear the REPLACEMENT's claim and record this sender's outcome
      // over it — marking the row sent while the replacement is still
      // delivering (Codex, PR #86).
      const { error } = await db
        .from("notifications")
        .update(patch)
        .eq("id", id)
        .eq("email_claim_token", stamp);
