 ...
 * omitted: no trial recorded, unparseable, or under Stripe's 48-hour floor.
 * Subscribing mid-trial keeps the days already promised — the subscription
 * starts billing when the trial was always going to end, not the moment the
 * operator adds a card, so subscribing early is never punished.
 */
export function trialEndSeconds(trialEndsAt: string | null, nowMs: number): number | null {
  ...
  if (t - nowMs < TRIAL_MIN_REMAINING_MS) return null;
...
export function operatorCheckoutParams(args: {
  ...
  return {
    ...
    // On both the session (read by platform-webhook's checkout arm) and the
    // subscription (read by the subscription arms if they arrive first).
    metadata,
    subscription_data: {
      metadata,
