...
describe("every selected column exists", () => {
  it("models the schema from the migrations", () => {
    ...
    expect([...(cols.get("notifications") ?? [])].sort()).toEqual([
      "body", "client_id", "created_at", "email_attempts", "email_last_error",
      "email_sent_at", "email_status", "id", "operator_id", "push_attempts",
      "push_last_error", "push_sent_at", "push_status", "read_at", "title",
      "type", "updated_at", "walk_id",
    ]);
