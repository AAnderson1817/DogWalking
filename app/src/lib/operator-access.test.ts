...
import { operatorAccess, type OperatorBillingState } from "./operator-access";

function billing(over: Partial<OperatorBillingState>): OperatorBillingState {
  return { trialEndsAt: null, platformSubscriptionStatus: "none", hasBilling: false, ...over };
}
...
describe("operatorAccess", () => {
  it("a live subscription is full access, whatever the trial says", () => {
    expect(operatorAccess(billing({ trialEndsAt: PAST, platformSubscriptionStatus: "active" }), NOW))
      .toBe("full");
    expect(operatorAccess(billing({ trialEndsAt: FUTURE, platformSubscriptionStatus: "active" }), NOW))
      .toBe("full");
  });

  it("past_due is grace — a banner while Stripe duns, never a wall", () => {
    expect(operatorAccess(billing({ trialEndsAt: PAST, platformSubscriptionStatus: "past_due" }), NOW))
      .toBe("grace");
  });
  ...
  it("inside the trial window everything short of a live subscription is still full", () => {
    for (const status of ["none", "cancelled", "paused"]) {
      expect(
        operatorAccess(billing({ trialEndsAt: FUTURE, platformSubscriptionStatus: status }), NOW),
        `status ${status}`,
      ).toBe("full");
    }
  });
  ...
  it("trial over with no live subscription is locked", () => {
    for (const status of ["none", "cancelled", "paused"]) {
      expect(
        operatorAccess(billing({ trialEndsAt: PAST, platformSubscriptionStatus: status }), NOW),
        `status ${status}`,
      ).toBe("locked");
    }
  });
  ...
  it("the boundary instant itself is locked (now >= trial end)", () => {
    expect(operatorAccess(billing({ trialEndsAt: FUTURE, platformSubscriptionStatus: "none" }),
  ...
  it("fails OPEN on anything unreadable", () => {
    ...
    expect(operatorAccess(billing({ trialEndsAt: "garbage", platformSubscriptionStatus: "none" }), NOW))
      .toBe("full");
    // A status value this code has never seen decides nothing.
    expect(operatorAccess(billing({ trialEndsAt: PAST, platformSubscriptionStatus: "brand_new" }), NOW))
      .toBe("full");
  });
  ...
  it("a null trial with no live subscription is locked, not open", () => {
    ...
    expect(operatorAccess(billing({ trialEndsAt: null, platformSubscriptionStatus: "none" }), NOW))
      .toBe("locked");
  });
});
