...
| SELECT on `auth.users`, directly or through a role it inherits | Nothing at apply time, which is why a replay cannot find it. At run time, definer functions read it as their owner: 0035's `fn_account_has_password` (the vault's set-a-password check), and 0054's `fn_my_email_status` and `fn_lift_my_email_suppression` (the portal's email switch), through the decision the two share, which is not itself a definer. Each raises `permission denied` on every call without it. The modelled role gets it through `service_role`; `db-push-check.sh` removes every path and shows `fn_account_has_password` and `fn_my_email_status` refused, the second standing for the shared decision. |
