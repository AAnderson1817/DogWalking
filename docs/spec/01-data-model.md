...
**credential_access_log** — `operator_id`, `credential_id uuid not null references access_credentials on delete restrict` (RESTRICT is load-bearing, not the default spelled out: spec 03's purge design relies on it to make a credential undeletable once it carries an audit row, so the trail cannot be erased by deleting what it describes — this line said `cascade` for the life of the project while the schema has been RESTRICT since 0002, and `smoke.sql` now asserts the delete rule), `accessed_by uuid not null` (auth uid), `purpose text null` — required and non-blank only for a read: 0002 declared it `not null check (length(trim(purpose)) > 0)`, and 0030 dropped the NOT NULL and replaced that check with `credential_access_log_purpose_required_for_read`, `check (action <> 'read' or (purpose is not null and length(trim(purpose)) > 0))`, because a revoke has no purpose and a failed re-auth has no credential the caller was entitled to name — `accessed_at timestamptz not null default now()`, plus the 0030 audit columns `action credential_action not null default 'read'`, `ip inet`, `user_agent text` and `walk_id uuid null references walks on delete restrict` (validated against the operator and the property). Append-only and immutable: `authenticated` holds SELECT only — 0030 revoked INSERT as well as UPDATE and DELETE, so a row is written only inside the definer function that performs the audited action (spec 03).
