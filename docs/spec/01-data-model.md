...
**credential_access_log** — `operator_id`, `credential_id references access_credentials on delete restrict` (RESTRICT is load-bearing, not the default spelled out: spec 03's purge design relies on it to make a credential undeletable once it carries an audit row, so the trail cannot be erased by deleting what it describes — this line said `cascade` for the life of the project while the schema has been RESTRICT since 0002, and `smoke.sql` now asserts the delete rule), `accessed_by uuid not null` (auth uid), `purpose text`, `accessed_at timestamptz default now()`, plus the 0030 audit columns `action credential_action not null default 'read'`, `ip inet`, `user_agent text` and `walk_id uuid null references walks on delete restrict` (validated against the operator and the property). Append-only and immutable: `authenticated` holds SELECT only — 0030 revoked INSERT as well as UPDATE and DELETE, so a row is written only inside the definer function that performs the audited action (spec 03).
