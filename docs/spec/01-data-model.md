...
All `id` columns `uuid default gen_random_uuid() primary key` unless noted. All tables get `created_at timestamptz default now()`; mutable tables also get `updated_at` (trigger-maintained). Every tenant table carries `operator_id uuid not null references operators(id)` with an index. Money = integer minor units, US cents: the `*_pence` column names are historical (`ops(locale)`) and hold cents. FKs `on delete restrict` unless noted.

## Enums

<!-- BEGIN GENERATED ENUM CATALOG -->

15 enum types, in migration order. Generated by
...
- `entry_method` (0001): `key_on_file` · `lockbox` · `smart_lock` · `door_code` · `buzzer_fob`
- `walk_status` (0001): `scheduled` · `in_progress` · `completed` · `cancelled` · `no_show`
- `ledger_entry_type` (0001): `grant` · `debit` · `adjust` · `rollover` · `expiry`
- `payment_type` (0001): `subscription` · `overage` · `topup`
- `payment_status` (0001, +0022): `pending` · `succeeded` · `failed` · `refunded` · `disputed`
- `client_status` (0001): `invited` · `active` · `paused` · `archived`
- `subscription_status` (0001): `none` · `active` · `paused` · `past_due` · `cancelled`
- `pet_size` (0001): `small` · `medium` · `large` · `giant`
- `rollover_policy` (0001): `none` · `capped` · `unlimited`
- `billing_cycle` (0001): `weekly` · `monthly`
- `notification_type` (0001, +0022, +0025, +0044): `walk_complete` · `low_credit` · `renewal_upcoming` · `payment_failed` · `walk_scheduled` · `walk_cancelled` · `payment_refunded` · `payment_disputed` · `subscription_cancelled` · `plan_changed_externally` · `payment_taken` · `card_saved`
- `email_delivery_status` (0029): `pending` · `sent` · `skipped` · `failed`
- `credential_action` (0030): `read` · `create` · `rotate` · `revoke` · `reauth_failed`
- `invite_claim_outcome` (0039): `claimed` · `not_found` · `already_claimed` · `expired` · `revoked` · `email_mismatch`
- `push_delivery_status` (0049): `pending` · `sent` · `skipped` · `failed`

<!-- END GENERATED ENUM CATALOG -->
...
**credential_access_log** — `operator_id`, `credential_id uuid not null references access_credentials on delete restrict` (RESTRICT is load-bearing, not the default spelled out: spec 03's purge design relies on it to make a credential undeletable once it carries an audit row, so the trail cannot be erased by deleting what it describes — this line said `cascade` for the life of the project while the schema has been RESTRICT since 0002, and `smoke.sql` now asserts the delete rule), `accessed_by uuid not null` (auth uid), `purpose text null` — required and non-blank only for a read: 0002 declared it `not null check (length(trim(purpose)) > 0)`, and 0030 dropped the NOT NULL and replaced that check with `credential_access_log_purpose_required_for_read`, `check (action <> 'read' or (purpose is not null and length(trim(purpose)) > 0))`, because a revoke has no purpose and a failed re-auth has no credential the caller was entitled to name — `accessed_at timestamptz not null default now()`, plus the 0030 audit columns `action credential_action not null default 'read'`, `ip inet`, `user_agent text` and `walk_id uuid null references walks on delete restrict` (validated against the operator and the property). Append-only and immutable: `authenticated` holds SELECT only — 0030 revoked INSERT as well as UPDATE and DELETE, so a row is written only inside the definer function that performs the audited action (spec 03). And that SELECT is a column list since 0056, leaving out `ip` and `user_agent`: they describe the walker's device, 0004's table-level grant had handed both to every client whose door the row names, and only the service role reads them now. A column added to this table later is withheld from API roles until a migration grants it by name.
...
**notifications** — `operator_id`, `client_id uuid null` (null ⇒ operator-facing), `type notification_type`, `title text`, `body text`, `walk_id uuid null`, `read_at timestamptz null`. Delivery state per channel (0029 email, 0049 push): `email_status email_delivery_status not null default 'pending'`, `email_attempts int not null default 0`, `email_sent_at timestamptz`, `email_last_error text`, and a `push_*` quartet of the same shape; plus the 0051 send claim — `email_claimed_at` / `push_claimed_at timestamptz` and `email_claim_token` / `push_claim_token uuid`, a lease and a fencing token per channel (spec 04). And `subject_client_id uuid null references clients on delete restrict` (0057): the client the row is ABOUT, whoever it is for. `client_id` decides who may read a row, so a row the walker reads carries NULL there and used to name its client only in its title, and `fn_purge_client` (deleting by `client_id`) left "<name> is low on credits" in the walker's inbox after an erasure. A trigger fills the subject from `client_id` or the walk when the row records one, and refuses a subject from another operator or one that disagrees with either; the writers whose rows record neither (`fn_notify_low_credit`, the Stripe webhook's walker notices) name it themselves. The trigger takes the walk row and then the client row `FOR KEY SHARE` (the 0037 order), so a notice written while its client is being erased waits for the erasure; an INSERT about an erased client is skipped (the trigger returns NULL, so the statement writes nothing and raises nothing), and an UPDATE that would move a row onto one raises. NULL only for a row about no client, such as the walker's own Sanpo subscription.
...
encryption secrets, and `last_error`; so this is a column-restricted table and `select("*")` on
