...
- `credits.ts` — client-side helpers: effective walk cost, low-credit predicate, ledger formatting, `committedCredits`. Two callers, two rules: the live weekend arithmetic prices ONLY the walk a client is still composing on Booking (no row yet, so no snapshot); a persisted walk is priced from its own `walks.cost_credits` (0043), live only when that is null — the coalesce `fn_walk_cost` performs, done inside `committedCredits` itself, whose row type REQUIRES the column so a caller cannot hand it rows without one and fall to live for every row (the callback is the live fallback only). Booking's committed sum used to run the live formula over persisted rows while the snapshot sat unread in them (`fix(walk-cost)`).
- `walk-cost.ts` — the weekend-surcharge arithmetic as a ZERO-IMPORT leaf, which `effectiveWalkCost` delegates to. A leaf on purpose: gate 8d (`scripts/check-walk-cost-parity.sh`) has a deno script read this file straight off the disk and compare its answers with `fn_walk_cost` and the `fn_snapshot_walk_price` trigger from one case list, and deno resolves imports by exact path — `credits.ts`, with its extensionless `./types` import, cannot be loaded that way (the `platform-price.test.ts` leaf-module precedent, one runtime further along). An import added to the leaf is what breaks the gate. The delegation itself is pinned by `credits.delegation.test.ts` (the leaf mocked to a sentinel): the gate reads the leaf while Booking calls `effectiveWalkCost`, so that one line is what makes the leaf "what Booking quotes".
