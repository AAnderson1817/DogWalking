...
**fn_walk_cost(p_walk uuid) → int** — `coalesce(walks.cost_credits, service_types.credit_cost + weekend_surcharge_credits if scheduled_date is Sat/Sun)`. STABLE, no lock. That weekend expression exists THREE times — here (the live fallback), in the `fn_snapshot_walk_price` trigger below (0044, what every new row is stamped with), and in `app/src/lib/walk-cost.ts` (what Booking quotes for a walk that has no row yet) — and until gate 8d nothing tied any two of them together. `scripts/check-walk-cost-parity.sh` now asks all three the one case list in `scripts/walk-cost-cases.txt` (weekdays, both weekend days, a leap day, a year boundary, a zero surcharge, and the US DST-transition weekends as ordinary calendar coverage — a DATE has no zone, so they pin nothing about a parse) and compares trigger with function, function with TypeScript, and all three with the expectation, plus the TypeScript runs with each other. The TypeScript side runs four times: under two fixed-offset zones either side of the day boundary (`Etc/GMT+12`, `Etc/GMT-14`), because in the runner's own zone a leaf reading the LOCAL day at a constant offset answers every case correctly, and under `America/Chicago` and `Australia/Sydney`, because a fixed offset has no DST and a leaf whose error is DST-dependent passes both — it is a day out only in a zone currently on daylight time, on rows dated in that zone's standard time, so a single DST zone pins it for half the year and the two chosen have complementary seasons (the run refuses by name if neither is on daylight time today); the SQL side reads the trigger's stamp and then NULLS the snapshot before calling this function, because the coalesce would otherwise hand back the trigger's answer and the function's own expression would be tied to nothing, and reads the function's answers through a join on the null having taken.
...
Until H32 this paragraph overstated what shipped: 0043 wrote the money-side snapshot and **nothing read it** — the overage charge still billed the live `plans.overage_rate_pence`, so the Settings-edit re-pricing this section describes as fixed was still live for the dollar amount. The charge path (`_lib/overage.ts`) now resolves, in the order the promises were made: the walk's `overage_rate_pence` snapshot (plan client), else its `visit_price_pence` snapshot (pay-per-visit client, 0044), else the live plan rate — only while the plan's subscription is LIVE (`active`/`paused`/`past_due`, mirroring 0044's SQL allow-list) — for walks carrying no snapshot at all: pre-0043 rows (the behaviour those rows shipped with), and the one post-0044 class that legitimately has none: a walk created while the client had no plan and the service no visit price, whose client subscribed before it completed. For that class the live rate is a deliberate fallback, not a snapshot — there was no agreed figure at creation, and refusing to bill a now-subscribed client's walk would be worse. A DEAD subscription's retained plan prices nothing (Codex finding on #76): `customer.subscription.deleted` keeps `plan_id`, and a cancelled client walked as pay-per-visit would otherwise bill a rate whose Stripe mandate died with the subscription. The same liveness rule governs the INSERT snapshot and the stamping backfill (0044). All three terms null → the honest refusal, operator-only.
...
**fn_debit_walk(p_walk uuid) → table(outcome text, cost int, new_balance int)** — locks the **walk, then the client** (0037, see the lock order below); cost := fn_walk_cost. If `balance >= cost`: ledger `debit` (−cost, walk_id), set `walks.credits_debited = cost`, `is_overage = false` → outcome `'debited'`. Else: NO ledger entry, balance untouched, set `credits_debited = 0`, `is_overage = true` → outcome `'overage'` (caller charges the WHOLE walk at the rate snapshotted at its creation — the plan overage rate, or the service's visit price for a no-plan client (H32) — invariant 3, never partial). Idempotent: if walk already debited or already flagged overage, returns prior outcome without re-applying.

**fn_adjust_credits(p_client uuid, p_amount int, p_note text) → int** — operator manual `adjust` (±). Rejects if result < 0.

**fn_apply_topup(p_client uuid, p_credits int, p_payment_intent_id text, p_amount_pence int) → boolean** (0044, review H32) — payment row + credit grant in ONE transaction, the `fn_apply_invoice_paid` shape, keyed on the PaymentIntent of a payment-mode Checkout Session. Called by stripe-webhook on `checkout.session.completed` with `mode='payment'`; returns false (and does nothing) when the intent was already applied — **including after a refund**, because `uq_topup_payment_per_intent` and the function's own check both cover `refunded`/`disputed`, closing the 0023 double-grant shape before it opens. The PI id is stamped on `payments.stripe_invoice_id` and (via `fn_grant_cycle_credits`) `credit_ledger.stripe_invoice_id`, so `fn_reverse_payment` claws back a refunded top-up proportionally with no topup-specific arm. Service-role only.
...
- **Walk completion**: complete-walk edge fn → `fn_debit_walk` → `'overage'` ⇒ charge-overage (off-session PaymentIntent, whole walk at the snapshotted rate — plan overage rate, or the visit price for a no-plan client) → payment row → report card + `walk_complete` notification, **and on a succeeded charge a `payment_taken` notification carrying the amount and the receipt** (review H12), whose wording follows the promise that priced the charge — the plan-credits sentence is false for a client who never had a plan.
...
2. **At booking.** `needsOverage` compared the walk's cost against the RAW
   ...
   done inside `committedCredits` itself: its row type REQUIRES the column, so
   a caller cannot hand it rows without one and fall to live for every row;
   ...
   `Booking.committed.test.tsx` pins both halves, `credits.delegation.test.ts`
   pins that `effectiveWalkCost` is the leaf gate 8d reads, and gate 8d ties
   the arithmetic itself to the two SQL copies.
...
- **Top-up** (first-class since 0044, review H32 — `payments.type='topup'` was a dead enum value until then): the operator opens a payment-mode Checkout from the client record naming the credits and the price; `checkout.session.completed` → `fn_apply_topup` records the payment and grants the credits atomically, and the paying card is saved (`setup_future_usage='off_session'`) so the client is chargeable per visit from then on. `fn_adjust_credits` remains the path for money taken outside Stripe.
- **Pay-per-visit** (0044, review H32): a client on no plan books and is materialized like anyone else (`subscription_status='none'` is in the allow-list); with credits from top-ups the walk debits normally, and without them `fn_debit_walk` flags overage and the walk is charged in full at the **visit price snapshotted at creation**. Card capture for a client who never tops up is a setup-mode Checkout carrying a mandate that names each priced service — refused (`visit_price_missing`) until a visit price exists to name, because a card saved under no stated terms is an off-session charge waiting to surprise somebody.
