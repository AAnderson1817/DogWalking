...
Until H32 this paragraph overstated what shipped: 0043 wrote the money-side snapshot and **nothing read it** — the overage charge still billed the live `plans.overage_rate_pence`, so the Settings-edit re-pricing this section describes as fixed was still live for the dollar amount. The charge path (`_lib/overage.ts`) now resolves, in the order the promises were made: the walk's `overage_rate_pence` snapshot (plan client), else its `visit_price_pence` snapshot (pay-per-visit client, 0044), else the live plan rate for walks carrying no snapshot at all — pre-0043 rows (the behaviour those rows shipped with), and the one post-0044 class that legitimately has none: a walk created while the client had no plan and the service no visit price, whose client subscribed before it completed. For that class the live rate is a deliberate fallback, not a snapshot — there was no agreed figure at creation, and refusing to bill a now-subscribed client's walk would be worse. All three null → the honest refusal, operator-only.
