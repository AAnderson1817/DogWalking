...
{ code } = error; if (code) throw error;` does. An assignment TARGET is written, not read — `box.error =
fallback` and `delete box.cause` inspect nothing. A carried key stops carrying
...
could have moved since the last definite write is UNKNOWN (a logical
assignment — `||=`, `&&=`, `??=` — settles nothing, since it may not assign;
whether its condition is statically settled is not asked, so such a name is
refused rather than resolved) — a write through an
...
`'none'` is deliberately included — that is a client who never subscribed, whom the operator may bill outside Sanpo. `past_due` is excluded by owner decision: service halts until payment clears. `fn_book_walk` carries the identical predicate, so client self-booking and nightly materialization cannot disagree. Idempotent via `uq_walks_schedule_origin` on `(schedule_id, origin_date)`, the target of its `ON CONFLICT DO NOTHING` (0012). The key is the day a walk was generated for, not the day it is scheduled: keyed on `scheduled_date`, as it was until 0012, a walk dragged to another day left its original day free, and the next run generated it again, to be billed twice.
