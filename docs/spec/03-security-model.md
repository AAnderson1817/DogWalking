...
**Only the UI stops most of a purged record being re-personalised.** The
...
| Add pet | `pets` redacted in place — **row kept** (0058); the database refuses a new one |
...
**0058 closed the pet half.** Keeping the pet rows as tombstones made a save
from a tab opened before the erasure a way to put a pet's name, medical notes
...
leaves the file. So `fn_purge_client` returns the objects to delete and the
operator's browser removes them (it already holds a folder-scoped delete
policy from 0004); `fn_purge_client_photos` then drops the last rows.

**The client's photos are the objects in the client's folders (0058).** Every
uploader writes `{operator}/{walk or pet}/{file}`, the layout the storage
policies read, so `fn_purge_client` returns every object in `storage.objects`
under the folder of each of the client's walks and pets — not the paths the
rows point at. The rows missed two kinds: a pet's earlier photos, because
replacing a photo uploads a new object and overwrites `pets.photo_path` while
nothing deletes the old one, and a walk photo whose row failed to insert after
its upload. Both survived every erasure before 0058. Each path starts with the
bucket that holds it: a walk photo and a pet photo have the same shape, and
the browser once guessed walk-photos for every pet photo, so no erasure of a
client whose pet had a photo reached its second phase.

**The database, not Storage, decides when the photos are gone.** `remove`
...
arrived later with nothing that named it (Codex on #106). Nor can the row be
changed back: "0058 closed the pet half", above.
